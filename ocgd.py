...
class OCACS(OCGD):
    ...
    def construct_master_variables_dict(self, write_to_file: bool = True) -> dict:
        ...
        if write_to_file:
            ...
            output_file_path = os.path.join(self.prj_dirs["codebook"], "ocacs_cb_vars.json")
    ...
    def acs_cb_variables(self, year: int, write_to_disk: Optional[bool] = False) -> pd.DataFrame:
        ...
        ocacs_cb_vars_path = os.path.join(self.prj_dirs["codebook"], "ocacs_cb_vars.json")
        ...
        with open(md_path, "w", encoding = "utf-8") as f:
            ...
            print(f"- Markdown documentation for {year} created at {md_path}\n")
    ...
    def get_var_list(self, year: int, category: str, output_format: str = "json") -> Union[Dict[str, Any], pd.DataFrame, list]:
        """
        Get the list of ACS variables for a given year and category.
        Args:
            year (int): The year of the ACS data.
            category (str): The category of variables to retrieve (e.g., "Demographic", "Social", "Economic", "Housing").
            output_format (str): The format in which to return the variable list. Options are "json", "dataframe", or "list". Defaults to "json".
        Returns:
            Union[Dict[str, Any], pd.DataFrame, list]: The variable list in the specified format.
        Raises:
            ValueError: If the year is not in the available ACS5 years, or if the output_format is not one of the allowed options.
        Examples:
            >>> var_list_json = get_var_list(2020, "Demographic", output_format="json")
            >>> var_list_df = get_var_list(2020, "Demographic", output_format="dataframe")
            >>> var_list_list = get_var_list(2020, "Demographic", output_format="list")
        Notes:
            This function retrieves the list of ACS variables for a given year and category from the master variables codebook JSON file and returns it in the specified format. The master variables codebook should be structured with years as top-level keys, followed by categories and variable information.
        """

        # Check if the year is in the available ACS5 years
        if year not in self.acs5_years:
            raise ValueError(f"Year {year} is not in the available ACS5 years: {self.acs5_years}")

        # The output_format parameter can be "json", "dataframe", or "list". If "json", return the variable list as a JSON object. If "dataframe", return the variable list as a pandas DataFrame with columns "variable" and "alias". If "list", return the variable list as a list of variable strings.
        if output_format not in ["json", "dataframe", "list"]:
            raise ValueError("output_format must be one of 'json', 'dataframe', or 'list'")
        
        # The path to the master variables codebook JSON file
        ...
        # Convert the variable list into a pandas DataFrame with columns "variable" and "alias"
        df_vars = pd.DataFrame.from_dict(cb_vars, orient = "index", columns = ["alias"]).reset_index().rename(columns = {"index": "variable"})

        # Get the list of variables from the DataFrame
        list_vars = list(df_vars["variable"].values)

        # Return the variable list in the specified output format
    ...
    def octl_to_sdf(self, year: int, geography: str):
        """
        Convert OCTL tables to spatial data frames for a given year and geography.
        Args:
            year (int): The ACS year (e.g., 2010, 2015, 2020).
            geography (str): The geography type (e.g., "CO", "CS", "PL", etc.).
        Returns:
            pd.DataFrame: A spatial data frame containing the OCTL data for the specified year and geography.
        Raises:
            None
        Example:
            >>> octl_sdf = octl_to_sdf(2010, "CO")
        Note:
            Ensure that the OCTL geodatabase for the specified year exists in the GIS directory.
        """

        print(f"Converting OCTL TigerLines feature class to spatial data frame for year: {year} and geography: {geography}...")

        # Set the OCTL geodatabase path
        octl_gdb_path = os.path.join(self.prj_dirs["gis"], f"octl_ocacs{year}.gdb")
